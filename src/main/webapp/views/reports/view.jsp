...
<head>
    ...
    <title>Отчет #<%= report.getId() %> - Power Grid Management</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1>📊 Отчет #<%= report.getId() %></h1>
        <p>Детальная информация об отчете по выполнению плана</p>
    </div>
    ...
    <div class="content">
        ...
        <div class="report-info">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    <span class="info-label">Заполнен:</span>
                    <span class="info-value">
                        <% if (report.getIsFilled() != null && report.getIsFilled()) { %>
                            <span class="filled-badge">✅ ЗАПОЛНЕН</span>
                        <% } else { %>
                            <span class="not-filled-badge">⚠️ НЕ ЗАПОЛНЕН</span>
                        <% } %>
                    </span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">👤 <%= report.getCreatedBy() != null ? report.getCreatedBy() : "—" %></span>
                </div>
            </div>
        </div>
        ...
        <div class="actions">
            <% if (report.getIsFilled() == null || !report.getIsFilled()) { %>
            <a href="<%= request.getContextPath() %>/reports?action=fill&id=<%= report.getId() %>" class="btn btn-warning">
                ✏️ Заполнить отчет
            </a>
            <% } %>

            <a href="<%= request.getContextPath() %>/reports?action=download&id=<%= report.getId() %>" class="btn btn-success">
                📥 Скачать отчет
            </a>
            ...
            <form action="<%= request.getContextPath() %>/reports" method="post" style="display: inline;">
                ...
                <button type="submit" class="btn" style="background: #e74c3c;"
                        onclick="return confirm('Удалить отчет #<%= report.getId() %>?')">
                    🗑️ Удалить отчет
                </button>
            </form>
